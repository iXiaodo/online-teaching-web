...
$(function(){
            ...
            success: function(res){
                if(res.success===1){
                    ...
                    if ($.isEmptyObject(data)){
                        ...
                        for(var i in data){
                            insert_html += '<li class="list-group-item" file-value="'+ data[i]['id']+'" url-value="'+ data[i]['url'] +'">'+data[i]['filename']+'<a href="'+data[i]['url']+'" class="badge iconfont download" style="cursor: pointer;" title="下载资料" target="_blank">&#xe684;</a>&nbsp;&nbsp;<span class="badge iconfont del" style="cursor: pointer;" title="删除资料" file-name="'+data[i]['filename']+'">&#xe713;</span></li>'
                            $("#file-list").html(insert_html);
                        }
                    }
                }
    ...
    $("#file_save_btn").off('click').on('click',function () {
        ...
        if(file_url===''||file_url===null||file_url===undefined){
            new GHAlert({
                content: '文件获取出错',
                type: "fail",
                time: 2000
    ...
    $("#file-list").on('click','span.badge.iconfont.del',function () {
        var th = $(this);
        var file_name = th.attr('file-name');
        var author = $("#sub-account-block").attr('author');
        var id = $(this).parent().attr("file-value");
